...
public class StringArrayTest {
	...
	public static void main(String[] args) {
		...
		String[] strArray = {"continue", "break", "switch", "array", "print"};		// 초기값 지정하며 배열 선언
		...
		String[] names = new String[5];			// 지정된 문자열이 없으므로 값은 nul(없다) 상태
		for (int i = 0; i < names.length; i++) {
			System.out.println("i = " + i + ", 문자열 = " + names[i]);
		}
		// String은 클래스, 문자열은 객체 -> names 배열의 주소, names[i]는 문자열 주소 - 둘 다 참조(주소)값을 저장합니ek.
		
//		System.out.println("charAt(0) : " + names[0].charAt(0) + "subString(2, 5) : " + names[0].substring(2, 5));		
		// 문자열이 null일 때 charAt등 메소드 실행은 오류 발생
		...
		for (int i = 0; i < names.length; i++) {
			names[i] = names[i].toUpperCase();		// names배열 요소들이 참조하던 주소가 변경됨
		}
