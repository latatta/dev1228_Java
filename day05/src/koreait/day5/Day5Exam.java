...
public class Day5Exam {

	public static void main(String[] args) {
			
/* 		오늘의 복습문제(Day5Exam.java) : 패스워드 규칙
			1) 8글자 이상이어야 한다.
			2) 특수문자 1개 이상 포함
...
*/
		...
		for (int i = 0; i < pw.length(); i++) {
			...
			if (temp >= 'A' && temp <= 'Z') 
				...
				// 추가 요구사항 처리
				if (temp == 60 || temp == 62 || temp == 40 || temp == 41 || temp == 59 ||
				temp == 37 || temp == 124 || temp == 32)
					isUseless = true;
				else 
					symbol_cnt++;
			}
		...
		// 추가 요구사항 : 사용할 수 없는 기호 (>, <, (, ), ;, %, \, 공백) 추가해보세요. nextLine() 메소드로 변경합니다.
		...
		if (pw.length() >= 8 && upper_cnt >= 1 && lower_cnt >= 1 
				&& numeric_cnt >= 1 && symbol_cnt >= 1 && !isUseless)
			isOk = true;
